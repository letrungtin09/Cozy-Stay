  ...
  const removeAnimation = (index: number | null) => {
    if (index != null) {
      ...
      prevElementRef.current
        ?.querySelector(".title")
        ?.classList.remove("animate-wiggle-1s");
      prevElementRef.current
        ?.querySelector(".topic")
        ?.classList.remove("animate-wiggle-1.2s");
      prevElementRef.current
        ?.querySelector(".description")
        ?.classList.remove("animate-wiggle-1.4s");
      prevElementRef.current
        ?.querySelector(".buttons")
        ?.classList.remove("animate-wiggle-1.6s");
    }
  };
  const renderAnimation = (index: number | null) => {
    if (index != null) {
      ...
      currentElementRef.current
        ?.querySelector(".title")
        ?.classList.add("animate-wiggle-1s");
      currentElementRef.current
        ?.querySelector(".topic")
        ?.classList.add("animate-wiggle-1.2s");
      currentElementRef.current
        ?.querySelector(".description")
        ?.classList.add("animate-wiggle-1.4s");
      currentElementRef.current
        ?.querySelector(".buttons")
        ?.classList.add("animate-wiggle-1.6s");
    }
  };
  const renderImage = (currentSlide: number) => {
    const imageCarouselElements = document.querySelector(
      `.imageCarousel-${currentSlide}`
    );
    const imageCarouselFace = document.querySelector(
      `.faceImageCarousel-${currentSlide}`
    );
      ...
      >
        <div>
          <div className="item relative w-full h-full">
            ...
            >
              <div className="title text-[3rem] font-bold transform -translate-y-[-50px] blur-[20px]">
                CĂN HỘ CAO CẤP
              </div>
              <div className="topic text-[1rem] font-bold leading-[1.3rem] text-main-color transform -translate-y-[-50px] blur-[20px]">
              ...
              <div className="description transform -translate-y-[-60px] blur-[20px]">
                ...
                thời gian lưu trú thoải mái. Căn hộ của chúng tôi nằm ngay trung
                tâm An Thượng. Tuy nhiên, là một con đường yên tĩnh và bạn là
        ...
        <div>
          <div className="item relative">
            ...
            >
              ...
              <div className="description transform -translate-y-[-60px] blur-[20px]">
                Chỗ lưu trú của chúng tôi tọa lạc ngay tại trung tâm thành phố
                Nha Trang, mang đến một không gian yên tĩnh và riêng tư cho du
                khách, trong khi vẫn có lợi thế tiện lợi và dễ dàng tiếp cận các
                hoạt động và tiện ích của khu vực.
              </div>
              <div className="buttons grid grid-cols-2-130px mt-[20px] grid-rows-[40px] gap-[5px] transform -translate-y-[-60px] blur-[20px]">
                <button className="bor border-0 bg-[#eee] tracking-[3px] font-[FontBanner] font-[500] text-[#000000]">
                  Xem Thêm
