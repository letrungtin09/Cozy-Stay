...
import PaymemtWallet from "@/app/wallet/modalPayment";
...
export default function Home() {
  ...
  const handleCodeChange = (status: boolean) => {
    if (checkPayed) {
      alert(
        "Vui lòng thanh toán hoặc hủy đơn chưa thanh toán trước khi tạo đơn mới"
      );
    } else {
  ...
  return (
    <LayoutHouseOwner>
      <div className="Wallet-container section-padding">
        <h1 className="title font-bold">Ví số dư của bạn</h1>
        <div className="content">
          <div className="info-wallet wallet-row">
            <p className="info-wallet-item line-bottom">
              ...
              <span className="total-money">
                {formatNumber(dataUser[0]?.totalMoney + moneyPlus)}đ
              </span>
            </p>
          ...
          <div className="history-wallet wallet-row overflow-auto max-h-80">
            ...
            {dataWallet.map(
              ({ id, transactionType, date, changeMoney, linkQr }) => {
                ...
                const bgColor = isPending
                  ...
                  ? "bg-[#14be79]"
                  : "bg-red-600";
                const transactionLabel = isPending
                  ? "Chưa thanh toán"
                  : isDeposit
                  ? "Nạp tiền"
                  : "Trừ tiền";
                ...
                const Content = (
                  <p className={`history-wallet-item history-w-col ${bgColor}`}>
                    <span className="id-history-w group relative">
                      {formatDate(date)}
                      <span className="absolute left-[100px]">
                        {" "}
                        {formatTime(date)}
                      </span>
                    </span>
                    <span className="tengiaodich-history-w">
                      {transactionLabel}
                    </span>
                    <span className="sotiengiaodich-history-w">
                      {isPending
                        ? formatNumber(changeMoney)
                        : `${amountPrefix}${formatNumber(changeMoney)}`}
                    </span>
                  </p>
                );
            ...
            {dataWallet && dataWallet.length <= 0 && (
              <p className="history-wallet-item history-w-col bg-[#14be79]">
                * Tài khoản chưa phát sinh giao dịch
              </p>
            )}
          </div>
        </div>
