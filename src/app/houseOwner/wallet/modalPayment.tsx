...
const PaymemtWallet: React.FC = ({
  ...
  const decrementAmount = () => {
    setAmount((prevAmount) =>
      prevAmount - 100000 >= 0 ? prevAmount - 100000 : 0
    );
  };
  ...
  const handlePayMent = async (numberId: any, moneyNumber: any) => {
    ...
    try {
      ...
      await addQrLink(
        newWallet,
        res.paymentLinkData.checkoutUrl,
        res.paymentLinkData.paymentLinkId
      );
  ...
  const handleSendWallet = (numberId: any, status: any, moneyNumber: any) => {
    ...
    ApiFunctions.postData(apiWallet, dataWallet).then((res) => {
  ...
  return (
    <>
      <div
        className="fixed z-[1000] w-full h-full top-0 w-screen h-screen bg-color-gray-0 opacity-[0.95] blur-md"
