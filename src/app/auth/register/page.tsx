...
export default function Register() {
    ...
    const handleRegister = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const apiUrl: string = `${process.env.NEXT_PUBLIC_API_URL!}/api/register`;
        ...
        if (userName !== '' && email !== '' && passWord !== '' && passWord === confirmPassword) {
            ...
            if (passWord == '') errorElement('Nhập password của bạn', passwordRef.current);
            else if (passWord !== confirmPassword) errorElement('Xác nhận lại mật khẩu không đúng', confirmPasswordRef.current);
            if (email == '') errorElement('Nhập email của bạn', emailRef.current);
            if (userName == '') errorElement('Nhập họ tên của bạn', fullNameRef.current);
        }
    }
    return (
        <>
            <div className="form-gadgets register bg-[#f7f7f7] pt-[100px] pb-[30px]">
                <div className="form-container register__form flex items-end justify-center">
                    <form onSubmit={handleRegister} className="form-user w-[390px] rounded-[5px] border-[1px] border-[#999] bg-[#ffffff] p-[20px] shadow-[0_0_5px_0_#e3e3e3]" action="">
                        ...
                        <div className="register__rule text-center mt-[15px] px-[20px] pb-[10px] border-b-[1px] border-[#e3e3e3]">
                            <span>Bạn đồng ý với
                                ...
                                <Link className="no-underline text-[#4ccd99] font-bold transition duration-[0.3s] hover:text-color-green-2" href="#"> Chính sách bảo mật
                                </Link> của chúng tôi
                            </span>
                        </div>
